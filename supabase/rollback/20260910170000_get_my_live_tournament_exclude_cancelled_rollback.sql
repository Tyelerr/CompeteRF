-- Rollback for 20260910170000_get_my_live_tournament_exclude_cancelled.sql
-- Restores the previous get_my_live_tournament (20260902120000) whose tournament-status
-- exclusion covered only 'completed'/'archived' (NOT 'cancelled'). Note: reverting re-exposes
-- the deleted-tournament-shows-as-live bug on the RPC path; the client-side deleted filter
-- (tournament-view.ts / use.profile.tournaments.ts) still guards it.
