-- 20260812120000_fix_update_pending_player_ambiguous.sql
-- Bugfix for 20260810/20260811: in the invitation UPDATE, unqualified `player_id`
-- was ambiguous between the RETURNS TABLE output column `player_id` and
-- player_invitations.player_id. Qualify it as public.player_invitations.player_id.
-- CREATE OR REPLACE — identical to the fixed body except that one WHERE clause.
...
begin
  ...
  if 'email' = any (v_changed) then
    update public.player_invitations
     ...
     where public.player_invitations.player_id = p_player_id
