-- 20260818150000_review_participation_chip.sql
-- Extend submit_tournament_review's participation gate to include chip_entries.
--
-- Context: the elimination bracket is entirely tournament_players-based (individuals) and the
-- ONLY doubles path in the app is CHIP (there is no scotch-doubles elimination bracket). Chip
-- players are represented in chip_entries: a TD-added chip SINGLES player has ONLY a chip_entries
-- row (no tournament_players / team-member row), and after start every chip entry (singles &
-- doubles, both members) carries p1_profile_id / p2_profile_id. The original gate accepted only
-- tournament_players + accepted team members, so a chip-entries-only player would be prompted
-- (the chip hub resolves them) yet blocked from submitting. Adding chip_entries closes that gap
-- and covers BOTH members of a chip doubles team. Everything else about the function is unchanged.
...
begin
  ...
  -- Must have actually participated: elim/self-reg single (tournament_players), accepted team
  -- member (chip doubles pre-start), OR a chip entry (chip singles/doubles, incl. both members).
  if not (
