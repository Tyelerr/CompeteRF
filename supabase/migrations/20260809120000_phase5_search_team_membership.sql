-- 20260809120000_phase5_search_team_membership.sql
-- Phase 5 follow-up: surface team membership in the registration search so the UI can
-- disable "already on a team" players AT SELECTION TIME (instead of the late error
-- from the team RPCs). Also exclude team members from Recent Players.
--
-- search_players_for_registration gains two columns:
--   on_team    boolean  — the player is on a (non-declined) team in this tournament
--   team_name  text     — that team's custom name, if any (optional UI context)
-- get_recent_players_for_registration gains the same columns (always false/null, since
-- it now also excludes team members) so both RPCs share the PlayerSearchResult shape.
--
-- Adding columns changes the return type, so both are DROP + CREATE (bodies otherwise
-- unchanged). Both stay SECURITY DEFINER, pinned search_path, manager-gated, masked
-- email / no phone. The team RPCs keep their own duplicate guard as the final safeguard.
...
comment on function public.search_players_for_registration(bigint, text, int) is
  'Phase 5: unified ACTIVE+PENDING search. is_registered = on this tournament (singles); on_team + team_name = on a non-declined team here (doubles). Masked email, no phone, manager-gated. Team RPCs keep the final duplicate guard.';

-- ── Recent players (now also excludes team members) ─────────────────────────────
...
comment on function public.get_recent_players_for_registration(bigint, int) is
  'Phase 5: recent players the caller registered, excluding anyone already on this tournament (as a player OR a team member). Masked email, no phone. on_team/team_name included for shape parity (always false/null here).';
