...
**Admin dashboard / TD tournament list routing (verified):** the **live admin landing is `app/(tabs)/admin/index.tsx`** — its `AdminScreen` is a role router that renders an **inline** dashboard component per role (`TDDashboard`, `BarOwnerDashboard`, `CompeteAdminDashboard`, `SuperAdminDashboard`) all defined in that one file. The TD's "My Tournaments" card pushes to **`app/(tabs)/admin/tournaments/tournament-director-manager.tsx`** (`TDTournamentsScreen`) — this is the **live TD tournament list**, and where TD-facing per-tournament actions (e.g. Manage Players) belong. It renders the shared `src/views/components/tournament/tournament.card.tsx` (`TournamentCard`, which has an optional `onManagePlayers` prop). **Legacy/unused, do not edit for live behavior:** `app/(tabs)/admin/tournament-management.tsx` (self-contained, nothing navigates to it), `app/(tabs)/admin/td-dashboard.tsx` (points at a non-existent `tournaments/td-tournaments` route), and `src/views/screens/admin/td.dashboard.screen.tsx` (static, no navigation).
