// app/(tabs)/community-guidelines.tsx
import { ACCEPTABLE_USE_POLICY } from "@/src/models/constants/legal-text";
      ...
      <View style={styles.wrapper}>
        <ScrollView style={styles.scroll} contentContainerStyle={[styles.scrollContent, isWeb && styles.scrollContentWeb]} showsVerticalScrollIndicator={false}>
          <View style={isWeb ? styles.webInner : styles.mobileInner}>
            <Text style={styles.pageTitle}>Community Guidelines</Text>
            <Text style={styles.pageSubtitle}>Acceptable Use and Content Policy</Text>
            <Text style={styles.lastUpdated}>Effective Date: {ACCEPTABLE_USE_POLICY.lastUpdated}</Text>
            <Text style={styles.preamble}>{ACCEPTABLE_USE_POLICY.preamble}</Text>
            {ACCEPTABLE_USE_POLICY.sections.map((section: LegalSection, index: number) => (
...
const styles = StyleSheet.create({
  ...
  pageSubtitle: { color: COLORS.textSecondary, fontSize: FONT_SIZES.md, marginBottom: SPACING.xs },
