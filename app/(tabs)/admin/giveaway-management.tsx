      ...
      <Modal visible={!!endEarlyTarget} transparent animationType="fade" onRequestClose={closeEndEarlyModal}>
        <Pressable style={m.overlay} onPress={closeEndEarlyModal}>
          <Pressable style={m.card} onPress={(e) => e.stopPropagation()}>
            ...
            <Text allowFontScaling={false} style={[m.title, { color: C.red }]}>End Giveaway Early?</Text>
            <Text allowFontScaling={false} style={m.giveawayName}>{endEarlyTarget?.name}</Text>
            ...
            <View style={m.btnRow}>
              <Pressable style={m.cancelBtn} onPress={closeEndEarlyModal} disabled={endingEarly}>
                <Text allowFontScaling={false} style={m.cancelBtnText}>Cancel</Text>
              </Pressable>
              <Pressable
      ...
      <Modal visible={vm.drawModalVisible} transparent animationType="fade" onRequestClose={vm.closeDrawModal}>
        <Pressable style={m.overlay} onPress={vm.closeDrawModal}>
          <Pressable style={m.card} onPress={(e) => e.stopPropagation()}>
            ...
            <Text allowFontScaling={false} style={m.title}>Draw Winner?</Text>
            <Text allowFontScaling={false} style={m.giveawayName}>{vm.selectedGiveaway?.name}</Text>

            <View style={[m.infoCard, { borderColor: C.blueBorder, backgroundColor: C.blueDim }]}>
              <Text allowFontScaling={false} style={m.infoCardLabel}>Eligible entries</Text>
              <Text allowFontScaling={false} style={[m.infoCardValue, { color: C.blue }]}>
            ...
            {(vm.selectedGiveaway?.entry_count || 0) === 0 && (
              <View style={[m.warnBox, { borderColor: C.redBorder, backgroundColor: C.redDim }]}>
                <Text allowFontScaling={false} style={[m.warnText, { color: C.red }]}>
            ...
            <View style={m.btnRow}>
              ...
              >
                {vm.processing !== null ? (
                  ...
                  <Text allowFontScaling={false} style={m.confirmBtnText}>Draw Winner 🎲</Text>
                )}
              </Pressable>
            </View>
          </Pressable>
      ...
      <Modal visible={vm.winnerModalVisible} transparent animationType="fade" onRequestClose={vm.closeWinnerModal}>
        <View style={m.overlay}>
          <View style={[m.card, { borderColor: C.goldBorder }]}>
            ...
            <Text allowFontScaling={false} style={[m.title, { color: C.gold }]}>Winner Selected!</Text>
            <Text allowFontScaling={false} style={m.giveawayName}>{vm.selectedGiveaway?.name}</Text>

            {vm.drawnWinner && (
              <View style={[m.winnerCard, { borderColor: C.goldBorder, backgroundColor: C.goldDim }]}>
                <Text allowFontScaling={false} style={m.winnerCardName}>{vm.drawnWinner.name}</Text>
                ...
                {[
                  ...
                  <View key={row.label} style={m.winnerDetailRow}>
                    <Text allowFontScaling={false} style={m.winnerDetailIcon}>{row.icon}</Text>
                    <Text allowFontScaling={false} style={m.winnerDetailLabel}>{row.label}</Text>
                    <Text allowFontScaling={false} style={[m.winnerDetailValue, { color: row.color }]}>{row.value}</Text>
                  </View>
                ))}
              </View>
            )}
            ...
            {vm.winnerFraudReport && vm.winnerFraudReport.riskLevel !== "clean" && (() => {
              ...
              return (
                <View style={[m.warnBox, { backgroundColor: bannerBg, borderColor: bannerBorder, marginBottom: scale(SP.md) }]}>
                  <Text allowFontScaling={false} style={[m.warnText, { color: bannerColor, marginBottom: scale(SP.xs) }]}>
                    {icon}  Fraud Review – {r.riskLevel.toUpperCase()} RISK
                  </Text>
                  {r.signals.map((sig, i) => (
                    <Text allowFontScaling={false} key={i} style={{ color: bannerColor, fontSize: moderateScale(FS.xs), marginTop: scale(2), opacity: 0.85 }}>
                      • {sig.description}
                    </Text>
                  ))}
                  {r.requiresManualReview && (
                    <Text allowFontScaling={false} style={{ color: bannerColor, fontSize: moderateScale(FS.xs), marginTop: scale(SP.sm), fontWeight: "700" }}>
                      Manual review recommended before awarding prize.
                    </Text>
                  )}
                  {r.requiresVerification && (
                    <Text allowFontScaling={false} style={{ color: bannerColor, fontSize: moderateScale(FS.xs), marginTop: scale(SP.xs), fontWeight: "600" }}>
            ...
            {vm.winnerFraudReport?.riskLevel === "clean" && (
              <View style={[m.warnBox, { backgroundColor: C.greenDim, borderColor: C.greenBorder, marginBottom: scale(SP.md) }]}>
                <Text allowFontScaling={false} style={{ color: C.green, fontSize: moderateScale(FS.sm), fontWeight: "600", textAlign: "center" }}>
            ...
            <Pressable
              style={{
                ...
                marginTop: scale(SP.sm),
              }}
              onPress={vm.closeWinnerModal}
            >
              <Text allowFontScaling={false} style={{ color: "#000000", fontSize: moderateScale(FS.lg), fontWeight: "800", letterSpacing: 0.5 }}>
                Done ✓
              </Text>
            </Pressable>
          </View>
      ...
      <Modal visible={vm.winnerDetailsModalVisible} transparent animationType="fade" onRequestClose={vm.closeWinnerDetailsModal}>
        <Pressable style={m.overlay} onPress={vm.closeWinnerDetailsModal}>
          <Pressable style={[m.card, { maxHeight: "80%" as any }]} onPress={(e) => e.stopPropagation()}>
            <Text allowFontScaling={false} style={m.title}>🏆  Winner Details</Text>
            <Text allowFontScaling={false} style={m.giveawayName}>{vm.selectedGiveaway?.name}</Text>

            {vm.loadingWinnerDetails ? (
              <ActivityIndicator size="large" color={C.blue} style={{ marginVertical: scale(SP.xl) }} />
            ) : vm.currentWinner ? (
              <ScrollView style={{ width: "100%" }} showsVerticalScrollIndicator={false}>
                <View style={[m.winnerCard, { borderColor: C.purple + "50", backgroundColor: C.purpleDim }]}>
                  <Text allowFontScaling={false} style={m.winnerCardName}>{vm.currentWinner.name}</Text>
                  <View style={m.winnerCardDivider} />
                  {[
                ...
                <View style={[m.infoCard, { borderColor: C.blueBorder, backgroundColor: C.blueDim, marginBottom: scale(SP.md) }]}>
                  <Text allowFontScaling={false} style={m.infoCardLabel}>Eligible for redraw</Text>
                  <Text allowFontScaling={false} style={[m.infoCardValue, { color: C.blue }]}>{vm.eligibleCount}</Text>
                </View>

                {vm.winnerHistory.length > 1 && (
                  <View style={m.historyBox}>
                    <Text allowFontScaling={false} style={m.historyTitle}>Draw History</Text>
                    {vm.winnerHistory.map((r) => (
                      <Text allowFontScaling={false} key={r.id} style={m.historyItem}>
              ...
              <Text allowFontScaling={false} style={m.bodyNote}>No winner on record.</Text>
            )}

            <View style={[m.btnRow, { marginTop: scale(SP.md) }]}>
              <Pressable style={m.cancelBtn} onPress={vm.closeWinnerDetailsModal}>
                <Text allowFontScaling={false} style={m.cancelBtnText}>Close</Text>
              </Pressable>
              {vm.currentWinner && vm.eligibleCount > 0 && (
                ...
                >
                  <Text allowFontScaling={false} style={m.confirmBtnText}>Redraw</Text>
                </Pressable>
              )}
            </View>
          </Pressable>
      ...
      <Modal visible={vm.redrawModalVisible} transparent animationType="fade" onRequestClose={vm.closeRedrawModal}>
        <Pressable style={m.overlay} onPress={vm.closeRedrawModal}>
          <Pressable style={m.card} onPress={(e) => e.stopPropagation()}>
            ...
            <Text allowFontScaling={false} style={[m.title, { color: C.amber }]}>Redraw Winner</Text>
            <Text allowFontScaling={false} style={m.bodyNote}>
              The current winner will be disqualified and a new winner drawn from the remaining{" "}
              <Text allowFontScaling={false} style={{ color: C.white, fontWeight: "700" }}>{vm.eligibleCount}</Text>{" "}
              eligible {vm.eligibleCount === 1 ? "entry" : "entries"}.
            </Text>

            <Text allowFontScaling={false} style={m.fieldLabel}>
              Reason for disqualification <Text allowFontScaling={false} style={{ color: C.red }}>*</Text>
            </Text>
            <TextInput
              style={m.reasonInput}
            ...
            <View style={m.btnRow}>
              ...
              >
                {vm.redrawing ? (
                  ...
                  <Text allowFontScaling={false} style={m.confirmBtnText}>Confirm Redraw</Text>
                )}
              </Pressable>
            </View>
          </Pressable>
...
const m = StyleSheet.create({
  ...
  title: { fontSize: moderateScale(FS.xl), fontWeight: "700", color: C.white, marginBottom: scale(SP.xs), textAlign: "center" },
  giveawayName: { fontSize: moderateScale(FS.md), color: C.lightGray, textAlign: "center", marginBottom: scale(SP.lg), fontWeight: "500" },
  ...
  infoCardLabel: { color: C.gray, fontSize: moderateScale(FS.sm) },
  infoCardValue: { fontSize: moderateScale(FS.xl), fontWeight: "700" },

  warnBox: { width: "100%", borderRadius: scale(10), padding: scale(SP.md), marginBottom: scale(SP.md), borderWidth: 1 },
  warnText: { fontSize: moderateScale(FS.sm), fontWeight: "600", textAlign: "center" },

  bodyNote: {
    color: C.gray,
    fontSize: moderateScale(FS.sm),
    textAlign: "center",
    lineHeight: 20,
    marginBottom: scale(SP.lg),
    width: "100%",
  },
  ...
  winnerCardName: { color: C.white, fontSize: moderateScale(FS.xl), fontWeight: "700", textAlign: "center" },
  winnerCardDivider: { height: scale(1), backgroundColor: C.cardBorder },
  winnerDetailRow: { flexDirection: "row", alignItems: "center", gap: scale(SP.sm) },
  winnerDetailIcon: { fontSize: moderateScale(15), width: scale(22), textAlign: "center" },
  winnerDetailLabel: { color: C.gray, fontSize: moderateScale(FS.sm), width: scale(70) },
  winnerDetailValue: { fontSize: moderateScale(FS.sm), fontWeight: "600", flex: 1 },
  ...
  historyTitle: { color: C.gray, fontSize: moderateScale(FS.xs), fontWeight: "700", textTransform: "uppercase", letterSpacing: 0.5, marginBottom: scale(SP.xs) },
  historyItem: { color: C.lightGray, fontSize: moderateScale(FS.sm), lineHeight: 20 },

  fieldLabel: { color: C.lightGray, fontSize: moderateScale(FS.sm), fontWeight: "500", marginBottom: scale(SP.sm), alignSelf: "flex-start" },
  reasonInput: {
    width: "100%",
    backgroundColor: C.card,
    borderWidth: 1,
    borderColor: C.cardBorder,
    borderRadius: scale(10),
    paddingVertical: scale(SP.md),
    paddingHorizontal: scale(SP.md),
    color: C.white,
    fontSize: moderateScale(FS.md),
    minHeight: scale(80),
    textAlignVertical: "top",
    marginBottom: scale(SP.lg),
  },

  btnRow: { flexDirection: "row", gap: scale(SP.md), width: "100%" },
  cancelBtn: {
    flex: 1,
  ...
  cancelBtnText: { color: C.lightGray, fontSize: moderateScale(FS.md), fontWeight: "600" },
  confirmBtn: {
    flex: 1,
    paddingVertical: scale(14),
    borderRadius: scale(12),
  ...
  confirmBtnText: { color: C.white, fontSize: moderateScale(FS.md), fontWeight: "700" },
