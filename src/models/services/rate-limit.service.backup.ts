...
export const rateLimitService = {
   ...
   * Uses upsert to prevent duplicate key errors from concurrent calls.
   */
  async getOrCreate(
    senderId: string,
    senderRole: SenderRole,
    ...
    // First-time sender: upsert to handle race condition
    // If two concurrent calls both see no row, the second upsert
    // will match on sender_id and return the existing row instead of crashing.
