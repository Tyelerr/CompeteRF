...
// Standard Expo Router tab bar height (49px bar + 34px home indicator safe area)
    ...
    <View style={s.dayPickerRow}>
      {DAYS_OF_WEEK_OPTIONS.map((day) => {
        ...
        return (
          <TouchableOpacity
            key={day.value}
            style={[s.dayChip, sel && s.dayChipSelected]}
            onPress={() => toggle(day.value)}
            disabled={disabled}
          >
            <Text style={[s.dayChipText, sel && s.dayChipTextSelected]}>
              {day.label.slice(0, 3)}
            </Text>
          </TouchableOpacity>
        );
  ...
  return (
    <TouchableOpacity style={s.toggleRow} onPress={onToggle} disabled={disabled}>
      ...
      <View style={[s.togglePill, value === true && s.togglePillYes, value === false && s.togglePillNo]}>
        <Text style={[s.togglePillText, value === true && s.togglePillTextYes, value === false && s.togglePillTextNo]}>
          {text}
        </Text>
      </View>
    </TouchableOpacity>
...
function AlertCard({ alert, onEdit, onDelete, onToggleActive }: {
  alert: SearchAlert;
  onEdit: () => void;
  onDelete: () => void;
  onToggleActive: () => void;
  ...
  return (
    <View style={s.alertCard}>
      <View style={s.cardTopRow}>
        <Text style={s.alertName} numberOfLines={1}>{alert.name}</Text>
        <TouchableOpacity
          style={[s.onOffBadge, alert.is_active ? s.onBadge : s.offBadge]}
      ...
      <Text style={s.alertDescription} numberOfLines={2}>{description}</Text>
      <Text style={s.matchInfo}>
        {alert.match_count} {alert.match_count === 1 ? "match" : "matches"}
        {alert.last_match_date ? ` · Last: ${formatDate(alert.last_match_date)}` : ""}
      </Text>
...
function CreateEditForm({ userId, alertId, onDone, onClose }: {
  userId: number;
  alertId: number | null;
  onDone: () => void;
  onClose: () => void;
    ...
    try {
      ...
      if (isEditMode && alertId) {
        await searchAlertService.updateAlert(alertId, {
          name: form.name.trim(), description,
          filter_criteria: criteria, is_active: form.isActive,
        });
      } else {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <View style={s.section}>
          ...
          <View style={s.fieldContainer} onLayout={(e) => { fieldOffsets.current["name"] = e.nativeEvent.layout.y; }}>
            <Text style={s.fieldLabel}>Alert Name <Text style={s.required}>*</Text></Text>
            <TextInput
              style={s.textInput}
          ...
          <View style={s.switchRow}>
            <View style={s.switchInfo}>
              ...
              <Text style={s.switchDescription}>{"Inactive alerts won\u0027t match new tournaments"}</Text>
            </View>
            <Switch
              value={form.isActive}
        ...
        <View style={s.section}>
          ...
          <View style={s.fieldContainer}>
            ...
            <Dropdown placeholder="Any Game Type" options={GAME_TYPE_OPTIONS}
              value={form.gameType} onSelect={(v: string) => updateField("gameType", v)} disabled={saving} />
            {form.gameType && !form.gameType.includes("scotch") && (
              <Text style={s.fieldHint}>💡 This will also match scotch doubles versions</Text>
            )}
          </View>
          <View style={s.fieldContainer}>
            <Text style={s.fieldLabel}>Tournament Format</Text>
            <Dropdown placeholder="Any Format" options={FORMAT_OPTIONS}
              value={form.tournamentFormat} onSelect={(v: string) => updateField("tournamentFormat", v)} disabled={saving} />
          </View>
          <View style={s.fieldContainer}>
            <Text style={s.fieldLabel}>Table Size</Text>
            <Dropdown placeholder="Any Table Size" options={TABLE_SIZE_OPTIONS}
              value={form.tableSize} onSelect={(v: string) => updateField("tableSize", v)} disabled={saving} />
          </View>
          <View style={s.fieldContainer} onLayout={(e) => { fieldOffsets.current["equipment"] = e.nativeEvent.layout.y; }}>
            ...
            <TextInput
              ...
              onFocus={() => scrollToField("equipment")}
              editable={!saving}
              autoCapitalize="words"
            />
          </View>
        ...
        <View style={s.section}>
          ...
          <View style={s.fieldContainer}>
            ...
            <Dropdown placeholder="Any State" options={stateOptions}
              value={form.state} onSelect={(v: string) => updateField("state", v)}
              disabled={saving} searchable searchPlaceholder="Search states..." />
          </View>
          <View style={s.fieldContainer}>
            ...
            <Dropdown
              ...
              disabled={saving || !form.state} searchable searchPlaceholder="Search cities..." />
          </View>
        </View>
        ...
        <View style={s.section}>
          ...
          <View style={s.rowFields}>
            <View style={[s.fieldContainer, s.fieldHalf]} onLayout={(e) => { fieldOffsets.current["entryFeeMin"] = e.nativeEvent.layout.y; }}>
              ...
              <TextInput style={s.textInput} placeholder="0"
                placeholderTextColor={COLORS.textMuted} value={form.entryFeeMin}
                onChangeText={(v) => updateField("entryFeeMin", v)}
                onFocus={() => scrollToField("entryFeeMin")}
                editable={!saving} keyboardType="numeric" />
            </View>
            <View style={[s.fieldContainer, s.fieldHalf]} onLayout={(e) => { fieldOffsets.current["entryFeeMax"] = e.nativeEvent.layout.y; }}>
              ...
              <TextInput style={s.textInput} placeholder="Any"
                placeholderTextColor={COLORS.textMuted} value={form.entryFeeMax}
                onChangeText={(v) => updateField("entryFeeMax", v)}
                onFocus={() => scrollToField("entryFeeMax")}
                editable={!saving} keyboardType="numeric" />
            </View>
          </View>
          <View style={s.fieldContainer} onLayout={(e) => { fieldOffsets.current["fargoMax"] = e.nativeEvent.layout.y; }}>
            ...
            <TextInput style={s.textInput} placeholder="e.g. 600 (leave blank for any)"
              placeholderTextColor={COLORS.textMuted} value={form.fargoMax}
              onChangeText={(v) => updateField("fargoMax", v)}
              onFocus={() => scrollToField("fargoMax")}
              editable={!saving} keyboardType="numeric" />
          </View>
        </View>
        ...
        <View style={s.section}>
          ...
          <ToggleRow label="Reports to Fargo" description="Tournament results reported to FargoRate"
            value={form.reportsToFargo} onToggle={() => cycleTriState("reportsToFargo")} disabled={saving} />
          <ToggleRow label="Calcutta" description="Tournament includes a Calcutta auction"
            value={form.calcutta} onToggle={() => cycleTriState("calcutta")} disabled={saving} />
          <ToggleRow label="Open Tournament" description="No skill cap restriction"
            value={form.openTournament} onToggle={() => cycleTriState("openTournament")} disabled={saving} />
        </View>

        <View style={s.section}>
          <Text style={s.sectionTitle}>Days of Week</Text>
          <Text style={s.sectionHint}>Leave empty to match any day</Text>
          <DayPicker selected={form.daysOfWeek}
            onChange={(days) => updateField("daysOfWeek", days)} disabled={saving} />
        </View>

        <View style={s.previewContainer}>
          <Text style={s.previewLabel}>Alert Preview</Text>
          <Text style={s.previewText}>
            {form.name.trim() || "Untitled Alert"} — {searchAlertService.generateAlertDescription(formToCriteria(form))}
          </Text>
        </View>
      </ScrollView>
      ...
      <View style={s.footer}>
        <TouchableOpacity
          style={[s.saveButton, (!form.name.trim() || saving) && s.buttonDisabled]}
          onPress={handleSave}
          disabled={!form.name.trim() || saving}
        >
          {saving
            ? <ActivityIndicator size="small" color={COLORS.white} />
            : <Text style={s.saveButtonText}>{isEditMode ? "Save Changes" : "Create Alert"}</Text>
          }
        </TouchableOpacity>
...
function AlertList({ userId, refreshKey, onOpenCreate, onOpenEdit, onClose }: {
  userId: number;
  refreshKey: number;
  onOpenCreate: () => void;
  ...
  useEffect(() => {
    setLoading(true);
    loadAlerts();
  }, [refreshKey, loadAlerts]);

  const handleToggleActive = (alert: SearchAlert) => {
    const action = alert.is_active ? "disable" : "enable";
      ...
      { text: "Cancel" },
      {
        ...
        onPress: async () => {
          try {
            ...
            Alert.alert("Error", `Failed to ${action} alert.`);
          }
        },
      },
    ]);
...
export function SearchAlertsModal({ visible, onClose }: SearchAlertsModalProps) {
  ...
  const innerContent = (
    <View style={{ flex: 1 }}>
      <AlertList
        ...
        onOpenCreate={() => openSheet(null)}
        onOpenEdit={(id) => openSheet(id)}
        onClose={onClose}
      />
      {sheetOpen && (
        <>
          ...
          />
          <Animated.View
            style={[s.sheetContainer, { transform: [{ translateY: sheetTranslate }] }]}
          >
            <CreateEditForm
              userId={userId}
  ...
  return (
    <Modal visible={visible} animationType="slide" transparent onRequestClose={onClose}>
      {/*
        paddingBottom = TAB_BAR_HEIGHT so the sheet stack sits above the tab bar.
        The tab bar is rendered outside the Modal tree and remains visible below.
      */}
...
const s = StyleSheet.create({
  ...
  // Mobile — paddingBottom keeps both sheets above the tab bar
  mobileOverlay: {
    flex: 1,
  ...
  // Create/edit sheet — absolute overlay inside mobileContainer
  sheetBackdrop: {
  ...
  closeButtonText: { color: COLORS.textSecondary, fontSize: 20, fontWeight: "600" },
  divider: { height: 1, backgroundColor: COLORS.border },

  loadingWrap: { flex: 1, alignItems: "center", justifyContent: "center", padding: SPACING.xl },
  loadingText: { color: COLORS.textSecondary, fontSize: FONT_SIZES.md, marginTop: SPACING.md },
  ...
  createButtonText: { color: COLORS.white, fontSize: FONT_SIZES.md, fontWeight: "600" },
  ...
  alertName: { fontSize: FONT_SIZES.lg, fontWeight: "700", color: COLORS.text, flex: 1, marginRight: SPACING.sm },
  onOffBadge: { paddingHorizontal: SPACING.md, paddingVertical: SPACING.xs, borderRadius: RADIUS.md },
  ...
  alertDescription: { fontSize: FONT_SIZES.md, color: COLORS.textSecondary, lineHeight: 20, marginBottom: SPACING.xs },
  matchInfo: { fontSize: FONT_SIZES.sm, color: COLORS.textMuted, marginBottom: SPACING.lg },
  actionRow: { flexDirection: "row", gap: SPACING.sm },
  actionButton: { flex: 1, paddingVertical: SPACING.sm, borderRadius: RADIUS.md, alignItems: "center", justifyContent: "center" },
  actionOutline: { borderWidth: 1, borderColor: COLORS.border },
  actionOutlineText: { fontSize: FONT_SIZES.sm, fontWeight: "600", color: COLORS.text },
  actionDelete: { borderWidth: 1, borderColor: COLORS.error + "60", backgroundColor: COLORS.error + "10" },
  actionDeleteText: { fontSize: FONT_SIZES.sm, fontWeight: "600", color: COLORS.error },

  emptyState: { flex: 1, alignItems: "center", justifyContent: "center", paddingVertical: SPACING.xl * 2, paddingHorizontal: SPACING.lg },
  emptyIcon: { fontSize: 60, marginBottom: SPACING.md },
  emptyTitle: { fontSize: FONT_SIZES.lg, color: COLORS.text, fontWeight: "600", marginBottom: SPACING.sm, textAlign: "center" },
  emptySubtitle: { fontSize: FONT_SIZES.md, color: COLORS.textSecondary, textAlign: "center", lineHeight: 20 },
  ...
  retryButton: { backgroundColor: COLORS.error, paddingHorizontal: SPACING.md, paddingVertical: SPACING.sm, borderRadius: RADIUS.sm },
  retryText: { color: COLORS.white, fontSize: FONT_SIZES.sm, fontWeight: "600" },

  formScrollContent: { padding: SPACING.md, paddingBottom: SPACING.xl },
  section: {
    marginBottom: SPACING.lg, backgroundColor: COLORS.backgroundCard,
    borderRadius: RADIUS.lg, padding: SPACING.lg,
    borderWidth: 1, borderColor: COLORS.border,
  },
  sectionTitle: { fontSize: FONT_SIZES.md, fontWeight: "700", color: COLORS.text, marginBottom: SPACING.md, textTransform: "uppercase", letterSpacing: 0.5 },
  sectionHint: { fontSize: FONT_SIZES.xs, color: COLORS.textMuted, marginBottom: SPACING.md, marginTop: -SPACING.sm },
  fieldContainer: { marginBottom: SPACING.md },
  fieldLabel: { fontSize: FONT_SIZES.sm, color: COLORS.text, fontWeight: "500", marginBottom: SPACING.sm },
  required: { color: COLORS.error },
  fieldHint: { fontSize: FONT_SIZES.xs, color: COLORS.primary, marginTop: SPACING.xs },
  textInput: {
  ...
  switchDescription: { fontSize: FONT_SIZES.xs, color: COLORS.textMuted, marginTop: 2 },
  ...
  toggleLabel: { fontSize: FONT_SIZES.sm, fontWeight: "500", color: COLORS.text },
  toggleDescription: { fontSize: FONT_SIZES.xs, color: COLORS.textMuted, marginTop: 2 },
  togglePill: { paddingHorizontal: SPACING.md, paddingVertical: SPACING.xs, borderRadius: RADIUS.sm, backgroundColor: COLORS.surface, borderWidth: 1, borderColor: COLORS.border, minWidth: 50, alignItems: "center" },
  ...
  togglePillText: { fontSize: FONT_SIZES.sm, fontWeight: "600", color: COLORS.textMuted },
  ...
  dayChip: { paddingHorizontal: SPACING.md, paddingVertical: SPACING.sm, borderRadius: RADIUS.md, backgroundColor: COLORS.surface, borderWidth: 1, borderColor: COLORS.border },
  dayChipSelected: { backgroundColor: COLORS.primary, borderColor: COLORS.primary },
  dayChipText: { fontSize: FONT_SIZES.sm, fontWeight: "500", color: COLORS.text },
  dayChipTextSelected: { color: COLORS.white },
  previewContainer: { backgroundColor: COLORS.surface, borderRadius: RADIUS.md, padding: SPACING.md, borderWidth: 1, borderColor: COLORS.border, marginBottom: SPACING.lg },
  previewLabel: { fontSize: FONT_SIZES.xs, fontWeight: "600", color: COLORS.textMuted, textTransform: "uppercase", marginBottom: SPACING.xs },
  previewText: { fontSize: FONT_SIZES.sm, color: COLORS.text, lineHeight: 20 },

  footer: { flexDirection: "row", gap: SPACING.sm, paddingHorizontal: SPACING.md, paddingTop: SPACING.sm, paddingBottom: SPACING.lg },
  saveButton: { flex: 1, paddingVertical: SPACING.md + 2, borderRadius: RADIUS.md, alignItems: "center", justifyContent: "center", backgroundColor: COLORS.primary },
  saveButtonText: { fontSize: FONT_SIZES.md, color: COLORS.white, fontWeight: "700" },
  cancelButton: { flex: 1, paddingVertical: SPACING.md + 2, borderRadius: RADIUS.md, alignItems: "center", justifyContent: "center", borderWidth: 1, borderColor: COLORS.error },
  cancelButtonText: { fontSize: FONT_SIZES.md, color: COLORS.error, fontWeight: "600" },
