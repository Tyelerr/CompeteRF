...
const getImageUrl = (t: any): string | null => {
  ...
  if (t.thumbnail?.startsWith("custom:"))
    return t.thumbnail.replace("custom:", "");
  ...
  const partial = Object.keys(map).find((k) =>
    t.game_type?.toLowerCase().includes(k),
  );
...
export function WebTournamentDetailOverlay({ id, onClose }: Props) {
  ...
  const chipRanges =
    isChip && Array.isArray(t.chip_ranges) && t.chip_ranges.length > 0
      ? t.chip_ranges
      : null;
            ...
            <View style={s.content}>
              ...
              <View style={s.topRow}>
                <View style={{ flex: 1, marginRight: 16 }}>
                  <View style={s.badges}>
                    <View style={s.idBadge}>
                      <Text style={s.idText}>ID: {t.id}</Text>
                    </View>
                    ...
                    <View style={s.fmtBadge}>
                      <Text style={s.fmtText}>
                        {t.tournament_format?.replace("_", " ")}
                      </Text>
                    </View>
                    {t.is_recurring && (
                      <View style={s.fmtBadge}>
                        <Text style={s.fmtText}>🔄 Weekly</Text>
                      </View>
                    )}
                  </View>
                ...
                <View style={{ alignItems: "center" }}>
                  ...
                  {imageUrl && (
                    ...
                    >
                      <Text style={s.viewImgText}>🔍 View Image</Text>
                    </TouchableOpacity>
                  )}
                </View>
              </View>
              ...
              {chipRanges && (
                <View style={s.section}>
                  <Text style={s.sectionTitle}>🎰 CHIP CHART</Text>
              ...
              <View style={s.section}>
                ...
                {t.table_size && (
                  <View style={s.row}>
                    ...
                    <Text style={s.val}>{t.table_size}</Text>
                  </View>
                )}
                <View style={s.row}>
                  <Text style={s.label}>Reports to Fargo:</Text>
                  <Text style={s.val}>{t.reports_to_fargo ? "Yes" : "No"}</Text>
                </View>
                <View style={s.row}>
                  <Text style={s.label}>Calcutta:</Text>
                  <Text style={s.val}>{t.calcutta ? "Yes" : "No"}</Text>
                </View>
                {!isChip && (
                  <View style={s.row}>
                    <Text style={s.label}>Open Tournament:</Text>
                    <Text style={s.val}>
                      {t.open_tournament ? "Yes" : "No"}
                    </Text>
                  </View>
                )}
                {t.max_fargo && !isChip && (
                  <View style={s.row}>
                    ...
                    <Text style={s.val}>{t.max_fargo}</Text>
                  </View>
                )}
                {t.race && !isChip && (
                  <View style={s.row}>
                    <Text style={s.label}>Race:</Text>
                    <Text style={s.val}>{t.race}</Text>
                  </View>
                )}
              </View>
              ...
              <View style={s.section}>
                ...
                <Text style={s.sectionText}>{t.venues?.address}</Text>
                <Text style={s.sectionText}>
                  {t.venues?.city}, {t.venues?.state} {t.venues?.zip_code}
                </Text>
                {t.venues?.phone && (
                  <Text
                    style={{
              ...
              >
                <TouchableOpacity
                  style={s.reportBtn}
                  onPress={() =>
                    report.openReportModal("tournament", t.id.toString())
                  }
                >
              ...
              >
                ...
                {t.venues?.venue || "an independent venue"}. Compete is not the
                organizer.
              </Text>
